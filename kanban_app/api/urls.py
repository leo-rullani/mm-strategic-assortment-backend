...
from .views import (
    ...
    BoardToolStatusDetailView,
    BoardToolStatusListView,
    DocumentContentView,
    DocumentDeleteView,
...
router.register(
    r"graphics-rapports", GraphicsRapportViewSet, basename="graphics-rapport"
)
...
urlpatterns = [
    ...
    # Shared CH/AT freshness/status table above the board
    path(
        "boards/<int:board_id>/tool-statuses/",
        BoardToolStatusListView.as_view(),
        name="board-tool-status-list",
    ),
    path(
        ("boards/<int:board_id>/tool-statuses/" "<str:market>/<slug:document_type>/"),
        BoardToolStatusDetailView.as_view(),
        name="board-tool-status-detail",
    ),
    ...
    path(
        "roster", roster, name="roster"
    ),  # bewusst ohne trailing slash: /api/roster?club=...
    # Router-basiert
